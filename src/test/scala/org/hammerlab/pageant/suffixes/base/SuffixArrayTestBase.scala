...
trait SuffixArrayBAMTest
  ...
  def testBam(num: Int, numPartitions: Int): Unit = {
    ...
    test(name) {
      ...
      // Read in `num` reads, concatenate them all separated by '$' delimiters, and convert to integers.
      val ots =
        sc.parallelize(
          sc
...
trait SuffixArrayDNATest
  extends SuffixArrayTestBase {

  self: FunSuite with Matchers =>
